...
from sqlalchemy import create_engine, text, MetaData, Table, Column, String, Float, Integer, inspect
import logging
import platform
import asyncio
import sys
...
# configure logging - ensure it is set before setting the event loop policy
...
try:
    ...
    def import_csv_to_db(csv_filename, table_name, if_exists='replace'):
        """
        Import a CSV file into the database
        
        Parameters:
            csv_filename (str): CSV file name (located in the csvs directory)
        ...
        try:
            ...
            if not os.path.exists(csv_path):
                ...
                current_dir_path = os.path.join(os.getcwd(), 'one_time_input', 'csvs', csv_filename)
                if os.path.exists(current_dir_path):
                    csv_path = current_dir_path
                    logger.info(f"File found in the current working directory: {csv_path}")
                else:
                    logger.error(f"File not found in the current working directory: {current_dir_path}")
                    return False
            ...
            # Convert IMO and MMSI columns to integer or string to avoid scientific notation
            logger.info("Converting numeric identifier columns to proper types...")
            for col in df.columns:
                # Use more flexible pattern matching to identify identifier columns
                if any(id_type in col.lower() for id_type in ['imo', 'mmsi']):
                    # Check if column contains numeric values
                    if pd.api.types.is_numeric_dtype(df[col]):
                        # Convert to integer if possible, otherwise to string
                        try:
                            df[col] = df[col].fillna(0).astype(int)
                            logger.info(f"Converted {col} to integer type")
                        except:
    ...
    def main():
        try:
            ...
            # csv_filename = "icct_scrubber_march_2025.csv"
            # table_name = "icct_scrubber_march_2025"
            csv_filename = "listing_20250315111657.csv"
            table_name = "wfr_ship_list"
            
            logger.info(f"Importing file {csv_filename} to table {table_name}")
            
            # import the CSV file to the database
            success = import_csv_to_db(csv_filename, table_name)
